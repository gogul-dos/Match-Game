...
class App extends Component {
  ...
  componentDidMount() {
    this.intervalId = setInterval(this.intervalFunction, 1000)
  }
  ...
  intervalFunction = () => {
    ...
    if (currentTime >= 1) {
      ...
      clearInterval(this.intervalId)
      this.toggleContainersVisibility()
    }
  }

  updateImage = () => {
    const randomIndex = Math.floor(Math.random() * imagesList.length)
    this.setState({imgItem: imagesList[randomIndex]})
  }

  buttonCategoryChanged = tabId => {
  ...
  tabItem = eachTab => {
    ...
    return (
      <TabItem
        key={eachTab.tabId}
        tab={eachTab}
        isSelected={selectedType === eachTab.tabId}
        onTabClick={this.buttonCategoryChanged}
      />
    )
  }

  imageSelected = eachImage => {
    const {imgItem} = this.state
    if (imgItem.id === eachImage.id) {
  ...
  filterListByType = type =>
    imagesList.filter(eachImage => eachImage.category === type)

  toggleContainersVisibility = () => {
    document.getElementById('gameContainer').classList.toggle('hide-container')
    document
      .getElementById('resultContainer')
      .classList.toggle('hide-container')
  }
  ...
  playAgainClicked = () => {
    this.setState(
      ...
      () => {
        console.log('State after playing again:', this.state)
        this.toggleContainersVisibility()
        this.updateImage()
        this.intervalId = setInterval(this.intervalFunction, 1000)
      },
  ...
  render() {
    const {currentTime, score, imgItem, selectedType} = this.state
    const {imageUrl} = imgItem
    const filteredList = this.filterListByType(selectedType)
    ...
    return (
      <div className="main-container">
        <nav className="navbar-container">
          ...
          <ul className="nav-elements">
            ...
            <div className="time-container">
              ...
              <li>
                <p className="span-items">{currentTime} sec</p>
              </li>
            </div>
          </ul>
        </nav>
        <div className="game-container" id="gameContainer">
          ...
          <ul>{tabsList.map(eachTab => this.tabItem(eachTab))}</ul>
          <div className="images-list-container">
            <ul className="unordered-list-image">
              {filteredList.map(eachImage => this.imageThumbnail(eachImage))}
            </ul>
          </div>
        </div>
        <div className="result-container hide-container" id="resultContainer">
          <img
